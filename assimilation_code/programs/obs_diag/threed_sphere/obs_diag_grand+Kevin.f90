...
! KDR prior%rmse is calculated for every ob, even if there ends up being 1 at that time.
!     If there's only 1, the value in rmse ends up being (still) the squared error.
...
! KDR We want rmse to have a non-MISSING value even when there is only 1 ob,
!     So that plot_rmse_xxx_profile.m can add up single obs at multiple times 
!     to get the grand statistics.   rmse has the (summed over 1) squared error.
!     This will result in the 'rmse' for 1 ob cases being artificially smaller
!     than the 2 ob case, which has the summed squared errors from 2 obs divided by 1.
!     But we shouldn't andor won't be looking at the rmse plotted by plot_rmse when the
!     number of obs = 1.  And the legend/grand statistics in plot_rmse_xxx_profile
!     will turn out right.
!     2020-4-14; actually the grand plot_rmse_xxx_profile grand calculation 
!                denormalized by Nused-1, so the times with 1 ob will NOT be included, 
!                regardless of the rmse value.
!           Original: 
!             prior%rmse(       iepoch, ilev, iregion, ivar) = MISSING_R8
!             prior%spread(     iepoch, ilev, iregion, ivar) = MISSING_R8
!             prior%totspread(  iepoch, ilev, iregion, ivar) = MISSING_R8
