...
!
! obs_seq - this is the actual obervartions.  e.g 80C in Denver, CO, and can
!  contain other data.
!
...
!   OBS_PRIOR_VAR_END
!
!  The parameters are really interger values. They make the code more human readable.
...
! in DART this is one long vector(:). This section is getting the X,Y,X, and variable info
...
!  not be calculated we skip to the next observation
   ...
   ! Can compute prior mean and variance of obs just once here
   ...
   STATE_UPDATE: do j = 1, num_close_states
      ...
      ! Update the state variable ensemble members
         ! Do update of state, correl only needed for varying ss inflate
          call update_from_obs_inc(obs_prior, obs_prior_mean, &
               obs_prior_var, obs_inc, &
               ens_handle%copies, grp_size, &
               increment, reg_coef, net_a)
      ...
      ens_handle%copies(1:ens_size, state_index) = ens_handle%copies(1:ens_size, state_index) + reg_factor * increment
...
! assimilated
   ...
   OBS_UPDATE: do j = 1, num_close_obs
      ...
      if(my_obs_indx(obs_index) > i) then
         ...
         ! Loop through and update ensemble members
          call update_from_obs_inc(obs_prior, obs_prior_mean, &
               obs_prior_var, obs_ens_handle%copies(:, obs_index), grp_size, &
                increment, reg_coef, net_a)
 
          ! Compute an information factor for impact of this observation on this state
         reg_factor = 1.0_r8  !HK one group so reg_factor=1

         ! Final weight is min of group and localization factors
         reg_factor = min(reg_factor, cov_factor)

          obs_ens_handle%copies(1:ens_size, obs_index) = obs_ens_handle%copies(1:ens_size, obs_index) + reg_factor * increment
