...
#PBS  -o Rean_SST0Z_branch.bld1
# #PBS  -o f.e21.FHIST_BGC.f09_025.CAM6assim.011.bld1
#PBS  -j oe 
#
# ---------------------
# Purpose
...
# Rean_2010.1-6_sst.25: single member for starting ensemble spin-ups: 
...
# Rean_2010.1-6_sst.25 files are stored in 
# setenv stagedir /glade/work/raeder/Models/CAM_init/FV.9x1.25_cam6.0.002
# Restart set for Rean_spinup_2010 is cobbled together in 
# setenv stagedir /gpfs/fs1/p/nsc/ncis0006/Reanalyses/Rean_spinup_2010/archive/rest/${reftimestamp}
...
set user_grid = "${user_grid}/config_grids+fv1+2deg_oi0.25_gland20.xml"
setenv sst_dataset \
   "/glade/work/raeder/Models/CAM_init/SST/avhrr-only-v2.2011-202001_0Z_filled_c200810.nc"
   # New SST valid and labeled at 12Z
   # "/glade/work/raeder/Models/CAM_init/SST/avhrr_v2.1-only-v2.201912-202109_12Z_filled_c211112.nc"
...
setenv sst_year_start 2011
setenv sst_year_end   2020
...
# If it is necessary to start a new case in a bit-for-bit identical way 
# to how an existing case would have continued, then 
#   + RUN_TYPE needs to be 'branch',
#   + the same filter used in the existing case must be put into $dartroot/models/cam-fv/work/filter,
#     The sizes of filter may not be enough; check dates and use cmp.
#   + additional files must be staged in RUNDIR (see stage_cesm_files, below);
#       cam_####.r. 
#       cam_####.rs. (these need to have the new CASE in their names, instead of the REFCASE.
#       cpl_####.r. files 
#       any requested restart history files (rh#)  (but hopefully not history files)
#   + the rpointer files must exist and have the right file names.
# 
# This will fix the CLM start, but may cause CAM to start differently than during an assimilation,
# since it's using(?) the restart file instead of the initial file.
...
# It can be every Mth day of the month, or every Mth weekday (e.g. Monday (at 00Z))
...
if (-f \${data_scratch}/run/rpointer.atm_0001) then
   ...
   # Now it's being used more widely, and should return the unaltered date.
   # The calling script can then derive what it needs.
...
# These are what we ended up using in the Reanalysis, after significant investigation.
      set cesm_data_dir = "/glade/p/cesmdata/cseg/inputdata/atm"
      set cesm_chem_dir = "/glade/p/cesmdata/cseg/inputdata/atm/cam/chem/emis/emissions_ssp370"
      set chem_root     = "${cesm_chem_dir}/emissions-cmip6-ScenarioMIP_IAMC-AIM-ssp370-1-1"
      set chem_dates    = "175001-210101_0.9x1.25_c20190222"

      echo " srf_emis_type                 = 'CYCLICAL'"   >> ${fname}
      echo " srf_emis_cycle_yr             = 2014"         >> ${fname}
...
#  This fails because a year can't be specified with 'FIXED' (!)
      ...
      echo " flbc_file  = '${cesm_data_dir}/waccm/lb/LBC_2014-2500_CMIP6_SSP370_0p5degLat_GlobAnnAvg_c190301.nc' "        >> ${fname}
...
#     This file that has yearly through 2014, not averaged.
      ...
      echo " tracer_cnst_datapath = '/glade/p/cesmdata/cseg/inputdata/atm/cam/tracer_cnst' "        >> ${fname}
      echo " tracer_cnst_file     = 'tracer_cnst_halons_3D_L70_2014-2101_CMIP6-SSP5-8.5_c190307.nc' "        >> ${fname}


      echo " ext_frc_type      = 'INTERP_MISSING_MONTHS' " >> ${fname}
      echo " ext_frc_specifier = "                         >> ${fname}
      echo "  'H2O    -> ${cesm_data_dir}/cam/chem/emis/elev/H2OemissionCH4oxidationx2_3D_L70_1849-2101_CMIP6ensAvg_SSP3-7.0_c190403.nc' "        >> ${fname}
   ...
   # For Lombardozzi
   echo " hist_fincl1 = 'TSA','ER','EFLX_LH_TOT','HR'"                  >> ${fname}
   echo " hist_fincl2 = 'CPHASE','GPP','GRAINC_TO_FOOD','GSSHALN',"     >> ${fname}
   echo "               'GSSUNLN','NPP','NPP_NUPTAKE','PLANT_NDEMAND'," >> ${fname}
   echo "               'QVEGT','TLAI'"                                 >> ${fname}
   echo " hist_nhtfrq = -6,-6"                                          >> ${fname}
   echo " hist_mfilt  = 1,1"                                            >> ${fname}
   echo " hist_avgflag_pertape = 'I','A'"                               >> ${fname}
   echo " hist_dov2xy = .true.,.false."                                 >> ${fname}
   echo " use_lch4 = .false."                                           >> ${fname}
   echo " model_year_align_popdens  = 1850"                             >> ${fname}
   echo " stream_year_first_popdens = 1850"                             >> ${fname}
   echo " stream_year_last_popdens  = 2100"                             >> ${fname}
   set clm_data = /glade/p/cesmdata/cseg/inputdata/lnd/clm2
   echo " stream_fldfilename_popdens = '${clm_data}/firedata/clmforc.Li_2018_SSP3_CMIP6_hdm_0.5x0.5_AVHRR_simyr1850-2100_c181205.nc'"                 >> ${fname}
   echo " ndep_taxmode           = 'cycle'"                             >> ${fname}
   echo " model_year_align_ndep  = 2015"                                >> ${fname}
   echo " stream_year_first_ndep = 2015"                                >> ${fname}
   echo " stream_year_last_ndep  = 2015"                                >> ${fname}

   echo " flanduse_timeseries = '${clm_data}/surfdata_map/landuse.timeseries_0.9x1.25_SSP5-8.5_78pfts_CMIP6_simyr1850-2100_c181209.nc'"                 >> ${fname}

#    echo "hist_fincl1 = 'TSA','CPHASE'"               >> ${fname}
#    echo "hist_fincl2 = 'ER','EFLX_LH_TOT','GPP','GRAINC_TO_FOOD','GSSHALN','GSSUNLN','HR','NPP','NPP_NUPTAKE','PLANT_NDEMAND','QVEGT','TLAI' "                               >> ${fname}
#    echo "hist_nhtfrq = -$stop_n,-$stop_n"                   >> ${fname}
#    echo "hist_mfilt  = 1,1"                          >> ${fname}
#    echo "hist_avgflag_pertape = 'I','A'"             >> ${fname}
#    echo "hist_dov2xy = .true.,.false."               >> ${fname}
...
#  echo "building_temp_method = 0 "                  >> ${fname}
   ...
   # ===========================================================================
   set fname = "user_nl_docn${inst_string}"
   # ===========================================================================
   echo '  taxmode = "limit"'                                      >> ${fname}
...
# preview_namelists needs to come before the modification of stream files.
./preview_namelists || exit 75

# The "old" avhrr v2 (file names avhrr-only-v2.YYYYMMDD.nc)
# The 1/4 degree avhrr SSTICE files' metadata says that the times are
# relative to 1978-01-01 00:00:00, but that the data are valid at the
# "center of the day".  The times in the file are whole days, not day.5,
# so that's inconsistent.  Fix that by setting an offset in the docn stream text files.
# BUT, that's not what we did in the f.e21.FHIST_BGC.f09_025.CAM6assim.011 case.
# The offset there is 0.

# The "new" avhrr v2.1 (file names oisst-avhrr-v02r01.YYYYMMDD.nc)
# Have consistent metadata; times counted as whole days from 1978-01-01 12Z, 
# which is day 0.

# Summary:
# 1) Make sure that preview_namelist has been run to propagate
#    the SSTICE pathname from env_run.xml to 
#    $CASEROOT/Casedocs/docn.streams.txt.prescribed_####
# 2) cp those files into $CASEROOT/user_{same_file_names}
# 3) They should not need modification for the new SST files.
# 4) If the SSTICE pathname needs to be changed, 
#    it must be changed in the user_docn.stream files
#    env_run.xml will be ignored for that variable.
#    (Or change env_run.xml and repeat 1) - 4)).

# echo ' '
# echo $sst_dataset | grep avhrr 
# if ($status == 0) then
#    pwd
#    ls -l CaseDocs/docn*
#    if ($status != 0) then
#       date
#       echo "ERROR: No CaseDocs/docn.stream files to modify"
#       exit 85
#    endif
#    @ inst = 1
#    while ($inst <= $num_instances)
#       set inst_string = `printf _%04d $inst`
#       sed -e "s#      0#      43200#"                        \
#           CaseDocs/docn.streams.txt.prescribed${inst_string} \
#           >   user_docn.streams.txt.prescribed${inst_string}
#       @ inst++
#    end
# endif
...
if ( \${CONTINUE_RUN} =~ TRUE ) then
   ...
   if (\$RUN_TYPE == 'branch') then
      # branch runs are like restart runs; they need the full set of restarts
      # and the usual rpointer files, so link the rest here.
      echo "RUN_TYPE = branch, so linking more files into RUNDIR"
      @ inst=1
      while (\$inst <= $num_instances)

         set inst_string = `printf _%04d $inst`

         ${LINK} -f ${stagedir}/${refcase}.cam\${inst_string}.r.${init_time}.nc  .

         # The rs file needs to have the current CASE in the name, not the ref case.
         # Data are not added to these files, so LINK is correct.
         ${LINK} -f ${stagedir}/${refcase}.cam\${inst_string}.rs.${init_time}.nc \
                               ${case}.cam\${inst_string}.rs.${init_time}.nc 

         ${LINK} -f ${stagedir}/${refcase}.cpl\${inst_string}.r.${init_time}.nc  .

         echo "${refcase}.clm2\${inst_string}.r.\${restart_time}.nc" >! rpointer.lnd\${inst_string}
         echo "${refcase}.cice\${inst_string}.r.\${restart_time}.nc" >! rpointer.ice\${inst_string}
         echo "${refcase}.cam\${inst_string}.r.\${restart_time}.nc"  >! rpointer.atm\${inst_string}
         if (mosart == 'rtm') then
            echo "${refcase}.rtm\${inst_string}.r.\${restart_time}.nc"  >! rpointer.rof\${inst_string}
         else if (mosart == 'mosart') then
            echo "${refcase}.mosart\${inst_string}.r.\${restart_time}.nc"  >! rpointer.rof\${inst_string}
         endif
         if ($num_instances > 1) then
            echo "${refcase}.cpl\${inst_string}.r.\${restart_time}.nc"     >! rpointer.drv\${inst_string}
            echo "${refcase}.docn\${inst_string}.r.\${restart_time}.nc"    >! rpointer.ocn\${inst_string}
            echo "${refcase}.docn\${inst_string}.rs1.\${restart_time}.bin" >> rpointer.ocn\${inst_string}
         endif

         @ inst ++
      end

      # At the moment I don't care about carefully limiting the number of links to \$NINST links,
      # So just link all the existing .rh files
      ${LINK} -f ${stagedir}/${refcase}.*.rh*.${init_time}.nc  .
      ...
      echo "RUN_TYPE = \$RUN_TYPE, so not linking more files"
   endif
   echo "All files set to run the FIRST experiment step using (ref)time" $init_time

endif

# I've never(?) seen this printed.  
# Other things are printed from stage_cesm_files into the .bld output file.
# Is there a problem with the escaping of status?  
# See what's happening with echo.
set echo
...
5) If this case needs to be under git control:
   a) move the CASEROOT directory to temp_name,
   b) on https://github.com/NCAR/DART_CASES make a branch named $case,
   c) clone that branch into a new, git-controlled CASEROOT,
   d) copy the contents of temp_name back into the git CASEROOT.

6)  Submit the job (and get mail when it starts and stops):
    ./case.submit -M begin,end

7)  After the job has run, check to make sure it worked.

8)  If the first cycle generated an ensemble by perturbing a single state,
    change input.nml as described in the instructions in it,
    to make cycles 2,...,N use the latest ensemble, unperturbed.

9)  To extend the run in $stop_n '$stop_option' steps, use xmlchange to
